    ...
    {/* fixed top-8 left-10 */}
      ...
      <div className='flex gap-4 items-center'>
            ...
            <ul className='flex gap-2 items-center sm_ooth'>
              {y > 200 ? <AiOutlineHome onClick={() => sc_section('Home')} size={40} className='cursor-pointer' style={{color: col.be_col}}/> : pe_ind()}
              {pe_ind()}
              {pe_ind()}
              {pe_ind()}
              {pe_ind()}
            </ul>
              <FaAngleDoubleRight size={40} className='cursor-pointer' style={{color: col.m_col}}/>
          </div>
      </div>
    </>
  )
